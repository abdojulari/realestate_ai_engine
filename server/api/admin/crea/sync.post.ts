...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    for (const creaProp of creProperties) {
      try {
        // CRITICAL: Fetch agent and office data PLUS the full property with
        // its Media collection. The bulk /Property feed (creaProp) does NOT
        // include Media — only the per-listing fetch does — so we must use
        // the enriched record from getPropertyWithAgentDetails for the
        // transform. Falling back to creaProp keeps non-image fields working
        // even if the enrichment call fails.
        let agentData = undefined
        let propertyForTransform = creaProp
        try {
