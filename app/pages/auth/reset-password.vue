<template>
  <div class="reset-container">
    <!-- Cinematic Sidebar (Left) — DeelBot promo -->
    <aside class="hero-sidebar d-none d-md-flex">
      <div class="parallax-container">
        <div class="parallax-bg" :style="{ backgroundImage: `url(${heroImages[currentImageIndex]})` }"></div>
        <div class="hero-overlay"></div>

        <div class="hero-content-wrapper pa-12">
          <div class="brand-badge mb-12">
            <img src="/images/logos/deelbot.png" alt="DeelBot" class="auth-logo-light" />
          </div>

          <div class="mt-auto">
            <transition name="slide-up" mode="out-in">
              <div :key="currentImageIndex">
                <h1 class="premium-display text-h2 text-white mb-6 leading-tight">
                  {{ heroContent[currentImageIndex]?.title }} <br/>
                  <span class="text-italic font-weight-light text-white opacity-80">
                    {{ heroContent[currentImageIndex]?.subtitle }}
                  </span>
                </h1>
              </div>
            </transition>

            <p class="text-h6 text-white opacity-70 font-weight-light leading-relaxed max-w-400">
              The intelligence layer behind Alberta's leading real estate brands. Powered by DeelBot.
            </p>

            <ul class="feature-list mt-10">
              <li v-for="(feat, i) in features" :key="i" class="feature-item">
                <span class="feature-bullet">
                  <v-icon size="14" color="white">{{ feat.icon }}</v-icon>
                </span>
                <div>
                  <div class="feature-title">{{ feat.title }}</div>
                  <div class="feature-sub">{{ feat.sub }}</div>
                </div>
              </li>
            </ul>
          </div>

          <div class="hero-footer mt-12 d-flex align-center justify-space-between">
            <a href="https://deelbot.com" target="_blank" rel="noopener" class="hero-link">
              <v-icon size="14" start>mdi-open-in-new</v-icon>
              Discover deelbot.com
            </a>
            <span class="text-caption text-white opacity-50">© {{ year }} DeelBot</span>
          </div>
        </div>

        <div class="carousel-indicators">
          <div
            v-for="(img, idx) in heroImages"
            :key="idx"
            class="indicator-dot"
            :class="{ active: idx === currentImageIndex }"
          ></div>
        </div>
      </div>
    </aside>

    <!-- Form Section (Right) -->
    <section class="form-section">
      <div class="form-inner-container">
        <div class="d-md-none text-center mb-10">
          <img src="/images/logos/deelbot.png" alt="DeelBot" class="auth-logo-mobile" />
        </div>

        <!-- Loading state -->
        <div v-if="checkingToken" class="state-card text-center">
          <v-progress-circular indeterminate color="black" size="36" width="3" class="mb-6" />
          <p class="text-body-1 text-grey-darken-1">Verifying your reset link…</p>
        </div>

        <!-- Invalid / expired token -->
        <div v-else-if="tokenInvalid" class="state-card text-center">
          <div class="error-icon">
            <v-icon size="40" color="white">mdi-link-variant-off</v-icon>
          </div>
          <h2 class="text-h4 font-weight-black tracking-tight mb-3">Link expired</h2>
          <p class="text-body-1 text-grey-darken-1 mb-6">
            This password reset link is no longer valid. Reset links expire after 30 minutes for your security.
          </p>
          <NuxtLink to="/auth/forgot-password">
            <v-btn color="black" size="x-large" rounded="pill" class="primary-btn shadow-xl">
              Request a new link
            </v-btn>
          </NuxtLink>
        </div>

        <!-- Success state -->
        <div v-else-if="resetSuccess" class="state-card text-center">
          <div class="success-icon">
            <v-icon size="40" color="white">mdi-check-circle-outline</v-icon>
          </div>
          <h2 class="text-h4 font-weight-black tracking-tight mb-3">Password updated</h2>
          <p class="text-body-1 text-grey-darken-1 mb-6">
            Your password has been changed and you are signed in. Redirecting you to your dashboard…
          </p>
          <v-progress-linear indeterminate color="black" rounded height="4" />
        </div>

        <!-- Main form -->
        <template v-else>
          <div class="mb-10 text-center text-md-left">
            <span class="kicker">Set a new password</span>
            <h2 class="text-h4 font-weight-black tracking-tight mb-2 mt-2">
              Welcome back<span v-if="firstName">, {{ firstName }}</span>.
            </h2>
            <p class="text-body-1 text-grey-darken-1 font-weight-light">
              Choose a strong new password to secure your DeelBot account.
            </p>
          </div>

          <v-form @submit.prevent="handleSubmit" autocomplete="off">
            <div class="form-group mb-5">
              <label class="premium-label mb-2">New password</label>
              <v-text-field
                v-model="newPassword"
                :type="showPassword ? 'text' : 'password'"
                density="compact"
                required
                variant="underlined"
                placeholder="At least 8 characters"
                hide-details="auto"
                class="premium-input"
                color="black"
                autocomplete="new-password"
                :disabled="loading"
                :append-inner-icon="showPassword ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"
                @click:append-inner="showPassword = !showPassword"
              />

              <!-- Strength meter -->
              <div v-if="newPassword" class="strength-row mt-3">
                <div class="strength-track">
                  <div class="strength-fill" :class="`strength-${strength.level}`" :style="{ width: strength.percent + '%' }"></div>
                </div>
                <span class="strength-label" :class="`strength-text-${strength.level}`">{{ strength.label }}</span>
              </div>

              <ul v-if="newPassword" class="checklist mt-3">
                <li :class="{ ok: rules.length }"><v-icon size="14">{{ rules.length ? 'mdi-check' : 'mdi-circle-small' }}</v-icon> 8+ characters</li>
                <li :class="{ ok: rules.upper }"><v-icon size="14">{{ rules.upper ? 'mdi-check' : 'mdi-circle-small' }}</v-icon> Uppercase letter</li>
                <li :class="{ ok: rules.number }"><v-icon size="14">{{ rules.number ? 'mdi-check' : 'mdi-circle-small' }}</v-icon> Number</li>
                <li :class="{ ok: rules.symbol }"><v-icon size="14">{{ rules.symbol ? 'mdi-check' : 'mdi-circle-small' }}</v-icon> Symbol (recommended)</li>
              </ul>
            </div>

            <div class="form-group mb-6">
              <label class="premium-label mb-2">Confirm new password</label>
              <v-text-field
                v-model="confirmPassword"
                :type="showConfirm ? 'text' : 'password'"
                density="compact"
                required
                variant="underlined"
                placeholder="Re-enter your new password"
                hide-details="auto"
                class="premium-input"
                color="black"
                autocomplete="new-password"
                :disabled="loading"
                :append-inner-icon="showConfirm ? 'mdi-eye-off-outline' : 'mdi-eye-outline'"
                @click:append-inner="showConfirm = !showConfirm"
                :error-messages="confirmPassword && !passwordsMatch ? ['Passwords do not match'] : []"
              />
            </div>

            <!-- Cloudflare Turnstile -->
            <div class="turnstile-wrapper mb-6">
              <p class="premium-label mb-2">Verify you're human</p>
              <div id="reset-turnstile-container" class="cf-turnstile"></div>
              <p v-if="turnstileError" class="text-caption text-error mt-1">{{ turnstileError }}</p>
            </div>

            <v-alert
              v-if="errorMessage"
              type="error"
              variant="tonal"
              class="mb-6 rounded-lg"
              closable
              @click:close="errorMessage = ''"
            >
              {{ errorMessage }}
            </v-alert>

            <v-btn
              type="submit"
              color="black"
              block
              size="x-large"
              class="primary-btn mb-6 shadow-xl"
              rounded="pill"
              :loading="loading"
              :disabled="!canSubmit"
            >
              Update Password & Sign In
            </v-btn>
          </v-form>
        </template>
      </div>
    </section>
  </div>
</template>

<script setup lang="ts">
import { ref, computed, onMounted, onUnmounted } from 'vue'
import { useAuthStore } from '~/stores/auth'

declare const turnstile: any

definePageMeta({
  layout: 'auth',
  guestOnly: true,
})

const route = useRoute()
const router = useRouter()
const authStore = useAuthStore()
const runtimeConfig = useRuntimeConfig()
const siteKey = (runtimeConfig.public.siteKey || '') as string
const { businessName } = useTenantSettings()

const token = computed(() => (route.query.token as string | undefined)?.trim() || '')

const checkingToken = ref(true)
const tokenInvalid = ref(false)
const firstName = ref<string | null>(null)

const newPassword = ref('')
const confirmPassword = ref('')
const showPassword = ref(false)
const showConfirm = ref(false)
const loading = ref(false)
const errorMessage = ref('')
const resetSuccess = ref(false)

const turnstileToken = ref<string | null>(null)
const turnstileVerified = ref(false)
const turnstileError = ref('')
const turnstileWidgetId = ref<string | null>(null)

const year = new Date().getFullYear()

const currentImageIndex = ref(0)
const heroImages = [
  'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?q=80&w=2000&auto=format&fit=crop',
  'https://images.unsplash.com/photo-1600607687940-4e524cb35d07?q=80&w=2000&auto=format&fit=crop',
  'https://images.unsplash.com/photo-1600047509807-ba8f99d2cdde?q=80&w=2000&auto=format&fit=crop',
]
const heroContent = [
  { title: 'A Fresh', subtitle: 'Start.' },
  { title: 'Secure', subtitle: 'By Design.' },
  { title: 'Welcome', subtitle: 'Home.' },
]
const features = [
  { icon: 'mdi-shield-check-outline', title: 'Bank-grade security', sub: 'Your password is hashed with bcrypt at 12 rounds.' },
  { icon: 'mdi-lightning-bolt-outline', title: 'AI-powered insights', sub: 'Real-time market valuations & lead scoring.' },
  { icon: 'mdi-handshake-outline', title: 'White-glove support', sub: 'Concierge onboarding for every brokerage.' },
]

let timer: any = null

const rules = computed(() => ({
  length: newPassword.value.length >= 8,
  upper: /[A-Z]/.test(newPassword.value),
  number: /\d/.test(newPassword.value),
  symbol: /[^A-Za-z0-9]/.test(newPassword.value),
}))

const strength = computed(() => {
  const r = rules.value
  const score = [r.length, r.upper, r.number, r.symbol].filter(Boolean).length
  if (!newPassword.value) return { level: 'empty', percent: 0, label: '' }
  if (score <= 1) return { level: 'weak', percent: 25, label: 'Weak' }
  if (score === 2) return { level: 'fair', percent: 55, label: 'Fair' }
  if (score === 3) return { level: 'good', percent: 80, label: 'Good' }
  return { level: 'strong', percent: 100, label: 'Strong' }
})

const passwordsMatch = computed(() => newPassword.value === confirmPassword.value)

const canSubmit = computed(() =>
  !loading.value &&
  rules.value.length &&
  passwordsMatch.value &&
  !!confirmPassword.value &&
  turnstileVerified.value
)

const initTurnstile = () => {
  if (!siteKey) {
    turnstileError.value = 'Human verification is not configured (missing NUXT_PUBLIC_SITE_KEY).'
    return
  }
  if (typeof turnstile === 'undefined') {
    setTimeout(initTurnstile, 200)
    return
  }
  turnstileWidgetId.value = turnstile.render('#reset-turnstile-container', {
    sitekey: siteKey,
    theme: 'light',
    callback: (t: string) => {
      turnstileToken.value = t
      turnstileVerified.value = true
      turnstileError.value = ''
    },
    'expired-callback': () => {
      turnstileToken.value = null
      turnstileVerified.value = false
      turnstileError.value = 'Verification expired. Please verify again.'
    },
    'error-callback': () => {
      turnstileError.value = 'Verification failed. Please try again.'
    },
  })
}

const resetTurnstile = () => {
  turnstileToken.value = null
  turnstileVerified.value = false
  if (siteKey && typeof turnstile !== 'undefined') {
    if (turnstileWidgetId.value) {
      try { turnstile.reset(turnstileWidgetId.value) } catch { turnstile.reset('#reset-turnstile-container') }
    } else {
      turnstile.reset('#reset-turnstile-container')
    }
  }
}

const validateToken = async () => {
  if (!token.value) {
    tokenInvalid.value = true
    checkingToken.value = false
    return
  }
  try {
    const res = await $fetch<{ valid: boolean; firstName: string | null }>('/api/auth/validate-reset-token', {
      method: 'POST',
      body: { token: token.value },
    })
    if (res?.valid) {
      firstName.value = res.firstName
    } else {
      tokenInvalid.value = true
    }
  } catch {
    tokenInvalid.value = true
  } finally {
    checkingToken.value = false
  }
}

const handleSubmit = async () => {
  if (!canSubmit.value) return
  loading.value = true
  errorMessage.value = ''
  try {
    const res = await $fetch<{ success: boolean; token: string }>('/api/auth/reset-password', {
      method: 'POST',
      body: {
        token: token.value,
        newPassword: newPassword.value,
        confirmPassword: confirmPassword.value,
        turnstileToken: turnstileToken.value,
      },
    })
    if (res?.token) {
      authStore.setToken(res.token)
      try { await authStore.checkAuth() } catch { /* fall through */ }
    }
    resetSuccess.value = true
    setTimeout(() => router.push('/'), 1800)
  } catch (error: any) {
    console.error('Reset password error:', error)
    const status = error?.response?.status || error?.statusCode
    const statusMessage = error?.data?.statusMessage || error?.statusMessage || ''
    if (status === 410 || /expired|invalid/i.test(statusMessage)) {
      tokenInvalid.value = true
    } else if (statusMessage) {
      errorMessage.value = statusMessage
      resetTurnstile()
    } else {
      errorMessage.value = 'We could not update your password. Please try again.'
      resetTurnstile()
    }
  } finally {
    loading.value = false
  }
}

onMounted(async () => {
  timer = setInterval(() => {
    currentImageIndex.value = (currentImageIndex.value + 1) % heroImages.length
  }, 6000)
  await validateToken()
  if (process.client && !tokenInvalid.value) initTurnstile()
})

onUnmounted(() => {
  if (timer) clearInterval(timer)
})

useHead({
  title: computed(() => `Reset Password — ${businessName.value || 'DeelBot'}`),
  meta: [
    { name: 'description', content: 'Choose a new password for your DeelBot account.' },
    { name: 'robots', content: 'noindex, nofollow' },
  ],
})
</script>

<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;1,400&family=Inter:wght@300;400;500;600;800&display=swap');

.reset-container {
  display: flex;
  min-height: 100vh;
  background: #ffffff;
  font-family: 'Inter', sans-serif;
  overflow: hidden;
}

.hero-sidebar { flex: 1.4; position: relative; overflow: hidden; }
.parallax-container { position: absolute; inset: 0; }
.parallax-bg {
  position: absolute;
  top: -10%; left: -10%;
  width: 120%; height: 120%;
  background-size: cover;
  background-position: center;
  transition: background-image 1.5s ease-in-out, transform 8s linear;
  animation: kenburns 20s infinite alternate;
}
@keyframes kenburns {
  from { transform: scale(1) translate(0, 0); }
  to { transform: scale(1.1) translate(-2%, -2%); }
}
.hero-overlay {
  position: absolute; inset: 0;
  background: linear-gradient(135deg, rgba(15,23,42,0.5) 0%, rgba(15,23,42,0.92) 100%);
  z-index: 1;
}
.hero-content-wrapper {
  position: relative; z-index: 2;
  height: 100%;
  display: flex; flex-direction: column;
}

.feature-list { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 18px; }
.feature-item { display: flex; align-items: flex-start; gap: 14px; color: #fff; }
.feature-bullet {
  width: 32px; height: 32px;
  border-radius: 999px;
  background: rgba(255,255,255,0.12);
  border: 1px solid rgba(255,255,255,0.2);
  display: inline-flex; align-items: center; justify-content: center;
  flex-shrink: 0;
}
.feature-title { font-weight: 700; font-size: 0.95rem; letter-spacing: -0.01em; }
.feature-sub { color: rgba(255,255,255,0.65); font-size: 0.825rem; line-height: 1.5; margin-top: 2px; }

.hero-link {
  color: rgba(255,255,255,0.85);
  font-size: 0.8rem; font-weight: 600;
  text-decoration: none;
  letter-spacing: 0.05em;
  text-transform: uppercase;
}
.hero-link:hover { color: #fff; }

.carousel-indicators {
  position: absolute; right: 40px; top: 50%;
  transform: translateY(-50%);
  display: flex; flex-direction: column; gap: 12px;
  z-index: 3;
}
.indicator-dot {
  width: 4px; height: 24px;
  background: rgba(255,255,255,0.2);
  border-radius: 4px;
  transition: all 0.6s cubic-bezier(0.4, 0, 0.2, 1);
}
.indicator-dot.active { background: #fff; height: 48px; }

.slide-up-enter-active, .slide-up-leave-active { transition: all 0.8s cubic-bezier(0.16, 1, 0.3, 1); }
.slide-up-enter-from { opacity: 0; transform: translateY(30px); }
.slide-up-leave-to { opacity: 0; transform: translateY(-30px); }

.form-section {
  flex: 1;
  display: flex; align-items: center; justify-content: center;
  padding: 60px;
  z-index: 2;
}
.form-inner-container { width: 100%; max-width: 460px; }

.kicker {
  display: inline-block;
  font-size: 0.7rem; font-weight: 800;
  letter-spacing: 0.2em; text-transform: uppercase;
  color: #0f172a; background: #f1f5f9;
  padding: 6px 12px; border-radius: 999px;
}
.premium-display { font-family: 'Playfair Display', serif; }
.text-italic { font-style: italic; }
.premium-label {
  font-size: 0.7rem; font-weight: 800;
  text-transform: uppercase; letter-spacing: 0.15em;
  color: #64748b;
}
.premium-input :deep(.v-field__input) { font-size: 1.1rem; padding-left: 0; }

.primary-btn {
  font-weight: 800 !important;
  text-transform: none !important;
  transition: transform 0.3s, box-shadow 0.3s;
}
.primary-btn:hover {
  transform: translateY(-2px);
  box-shadow: 0 10px 30px rgba(0,0,0,0.15) !important;
}

.strength-row { display: flex; align-items: center; gap: 12px; }
.strength-track {
  flex: 1; height: 6px;
  background: #f1f5f9;
  border-radius: 999px;
  overflow: hidden;
}
.strength-fill {
  height: 100%;
  border-radius: 999px;
  transition: width 0.3s ease, background 0.3s ease;
}
.strength-weak { background: #ef4444; }
.strength-fair { background: #f59e0b; }
.strength-good { background: #3b82f6; }
.strength-strong { background: #10b981; }
.strength-label {
  font-size: 0.75rem; font-weight: 700; letter-spacing: 0.05em;
  text-transform: uppercase; min-width: 60px; text-align: right;
}
.strength-text-weak { color: #ef4444; }
.strength-text-fair { color: #f59e0b; }
.strength-text-good { color: #3b82f6; }
.strength-text-strong { color: #10b981; }

.checklist {
  list-style: none; padding: 0; margin: 0;
  display: grid; grid-template-columns: 1fr 1fr; gap: 6px 16px;
}
.checklist li {
  font-size: 0.8rem; color: #94a3b8;
  display: inline-flex; align-items: center; gap: 6px;
}
.checklist li.ok { color: #10b981; font-weight: 600; }

.state-card { padding: 8px 0; }
.success-icon, .error-icon {
  width: 76px; height: 76px;
  border-radius: 999px;
  display: inline-flex; align-items: center; justify-content: center;
  margin-bottom: 20px;
}
.success-icon {
  background: linear-gradient(135deg, #10b981 0%, #059669 100%);
  box-shadow: 0 12px 32px rgba(16, 185, 129, 0.35);
}
.error-icon {
  background: linear-gradient(135deg, #ef4444 0%, #dc2626 100%);
  box-shadow: 0 12px 32px rgba(239, 68, 68, 0.3);
}

.auth-logo-light {
  height: 52px; width: auto; object-fit: contain;
  filter: brightness(0) invert(1);
}
.auth-logo-mobile { height: 48px; width: auto; object-fit: contain; }

.turnstile-wrapper { display: flex; flex-direction: column; align-items: flex-start; }
.tracking-tight { letter-spacing: -0.02em; }

@media (max-width: 960px) {
  .form-section { padding: 30px; }
}
@media (max-width: 480px) {
  .form-section { padding: 24px 20px; }
  .checklist { grid-template-columns: 1fr; }
}
</style>
